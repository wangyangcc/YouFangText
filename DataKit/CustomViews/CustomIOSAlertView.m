...
- (UIView *)createContainerView
{
    ...
    if (self.alertViewCornerRadius > 0) {
        cornerRadius = self.alertViewCornerRadius;
    }
...
- (void)addButtonsToView: (UIView *)container
{
    ...
    for (int i=0; i<[buttonTitles count]; i++) {
        ...
        [closeButton setTitleColor:MMRGBColor(121, 126, 137) forState:UIControlStateNormal];
        [closeButton setBackgroundImage:MMGrayBackground forState:UIControlStateHighlighted];
//        [closeButton setTitleColor:[UIColor colorWithRed:0 green:0 blue:0 alpha:0.5f] forState:UIControlStateHighlighted];
    ...
    for (int i=0; i<[buttonTitles count]; i++) {
        ...
        if (i != [buttonTitles count] - 1) {
            UIImageView *fengeView = [[UIImageView alloc] initWithFrame:CGRectMake((i +1)* [container width]/[buttonTitles count], container.bounds.size.height - buttonHeight + (buttonHeight - 10 )/2, 0.5, 10)];
            fengeView.image = [UIImage imageNamed:@"img_verLine"];
            [container addSubview:fengeView];
        }
    }
}
...
- (void)addBlurViewIn
{
    self.blurView = [[FXBlurView alloc] initWithFrame:[[[UIApplication sharedApplication] keyWindow] bounds]];
    self.blurView.dynamic = NO;
    self.blurView.blurRadius = 10;
    self.blurView.tintColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.5];
    //添加单击取消方法
    UITapGestureRecognizer *tapGes = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(blurViewInTaped)];
 ...
 *  背景模态试图点击
 */
- (void)blurViewInTaped
{
    if (self.cancelBlurViewTaped == NO) {
        [self close];
    }
}
