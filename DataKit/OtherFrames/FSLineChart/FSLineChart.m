...
- (void)setDefaultParameters
{
    ...
    _axisHeight = self.frame.size.height - _margin - 5;
...
- (void)setChartData:(NSArray *)chartData
{
    ...
    if(_labelForValue) {
        ...
        for(int i=_verticalGridStep;i>0;i--) {
...
- (UILabel*)createLabelForValue: (NSUInteger)index
{
    ...
    //画 == 0 的线
    if (index == _verticalGridStep) {
        p = CGPointMake(0, _axisHeight + _margin);
        text = @"0";
    }
    if(!text)
    {
    ...
    UILabel* label = [[UILabel alloc] initWithFrame:CGRectMake(0, p.y, width + 2, 14)]; //p.x - xOffset
    label.text = text;
...
//        }
        ...
        for(int i=0;i<_verticalGridStep + 1;i++) {
            ...
            //画 == 0 的线
            if (i == _verticalGridStep) {
                CGContextSetStrokeColorWithColor(ctx, [_innerGridColor CGColor]);
                CGContextSetLineWidth(ctx, _innerGridLineWidth);
                
                CGPoint point = CGPointMake(_margin/2, _axisHeight + _margin);
                
                CGContextMoveToPoint(ctx, point.x, point.y);
                CGContextAddLineToPoint(ctx, _axisWidth + _margin*3/2, point.y);
                CGContextStrokePath(ctx);
                continue;
            }
            //end
...
//            }
//            
//            CGPoint point = CGPointMake(_margin/2, (i) * _axisHeight / _verticalGridStep + _margin);
//            
//            CGContextMoveToPoint(ctx, point.x, point.y);
//            CGContextAddLineToPoint(ctx, _axisWidth + _margin*3/2, point.y);
//            CGContextStrokePath(ctx);
...
//        fillLayer.lineJoin = kCALineJoinRound;
        ...
        textLayer.backgroundColor = _dataPointBackgroundColor.CGColor;
        if (self.isDisplaySleep) {
            textLayer.string = [NSString stringWithFormat:@"%@",[NSString getSleepHourTimeWithMinuteTimeT:_data[i]]];//卡
        }
        else {
            textLayer.string = [NSString stringWithFormat:@"%@",_data[i]];//卡
        }
        textLayer.fontSize = 10;
        textLayer.alignmentMode = @"center";
        textLayer.truncationMode = @"end";
        textLayer.contentsScale = [UIScreen mainScreen].scale;
        textLayer.foregroundColor = _dataPointColor.CGColor;
        textLayer.frame = CGRectMake(p.x - 20, p.y - 20, 40, 15);
        textLayer.font = (__bridge CFTypeRef)[UIFont systemFontOfSize:10];
        //end
...
{
//    NSInteger currentBei = _max/kLineNumber;
//    if (_max - currentBei*1.5*kLineNumber > 0.000001) {
//        return (currentBei + 1)*kLineNumber;
//    }
//    return currentBei*kLineNumber;
    return MAX(_max, 0);
}
...
- (CGPoint)getPointForIndex:(NSUInteger)idx withScale:(CGFloat)scale
{
    ...
    CGFloat value = (scale == 0 ? 0 : [number floatValue] * scale);
    if(_data.count < 2) {
        return CGPointMake(_margin, _axisHeight + _margin - value);
    } else {
        return CGPointMake(_margin + idx * (_axisWidth / (_data.count - 1)), _axisHeight + _margin - value);
    }
}
