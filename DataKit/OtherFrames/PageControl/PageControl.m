...
- (void)layoutControls
{
	...
	{
		mainFrame.size.height = _image.size.height*self.onePageZoom + (2 * _padding);
		mainFrame.size.width = _image.size.width*self.onePageZoom * _numberOfPages + (_padding * (_numberOfPages + 2));
		mainFrame.origin.x = floor((mainFrame.origin.x + (self.frame.size.width / 2.0f)) - mainFrame.size.width / 2.0f);
	}
	else 
	{
		mainFrame.size.width = _image.size.height*self.onePageZoom + (2 * _padding);
		mainFrame.size.height = _image.size.width*self.onePageZoom * _numberOfPages + (_padding * (_numberOfPages + 2));
	...
	if (_orientation == PageControlOrientationLandscape)
	{
		...
		for (UIImageView *v in pages)
		{
            ...
            rect.size = CGSizeMake(v.image.size.width*self.onePageZoom, v.image.size.height*self.onePageZoom);
			v.frame = rect;
